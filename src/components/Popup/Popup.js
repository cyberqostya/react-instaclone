...
const Popup = (props) => {
  ...
  function handlerButton(evt) {
    ...
    if ( popupData.buttonName === 'Сохранить' ) {
      ...
      props.cards.setNewCards([...props.cards.newCards,
        {
            ...
            popupData.formName === 'edit' ?
              <>
                <div className="popup__input-field">
                  <input type="text" name="name" className="popup__input" placeholder="Name" minLength="2" maxLength="30" required onInput={(evt) => {handlerText(evt.target, setInputFirstValidity)}} />
                  <p className="popup__error-message"></p>
                </div>
                <div className="popup__input-field">
                  <input type="text" name="job" className="popup__input" placeholder="About yourself" minLength="2" maxLength="30" required onInput={(evt) => {handlerText(evt.target, setInputSecondValidity)}} />
                  <p className="popup__error-message"></p>
                </div>
              </>
            :
              <>
                <div className="popup__input-field">
                  <input className="popup__input" name="title" type="text" placeholder="Title" minLength="2" maxLength="30" required onInput={(evt) => {handlerText(evt.target, setInputFirstValidity)}} />
                  <p className="popup__error-message"></p>
                </div>
                <div className="popup__input-field">
                  <input className="popup__input" name="link" type="text" placeholder="Link on image" required onInput={(evt) => {handlerLink(evt.target, setInputSecondValidity)}}   />
                  <p className="popup__error-message"></p>
                </div>
              </>
          }

            <button className={popupData.formName==='edit' ? "popup__button popup__edit-button button" : "popup__button button"} disabled={checkValidity()} onClick={handlerButton}>{popupData.buttonName}</button>
          </form>
        </div>
      </div>
    );
    ...
    return (
      <div className="popup" onClick={closePopup}>
        <div className="popup__container">
          <img className="popup__image" src={popupData.url} alt="Beautyfull pic" />
        </div>
      </div>
    );
